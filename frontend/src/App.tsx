...
export function App() {
  ...
  async function attachSource(sourceId: number) {
    ...
    try {
      ...
      setNotice("Источник подключён к каналу");
    } catch (exc) {
      ...
      <main className="workspace">
        ...
        {section === "targets" && (
          <section className="split-admin">
            <div className="table-card">
              <div className="table-head"><h2>Каналы</h2><div className="table-actions"><span>{targets.length} всего</span><button className="primary compact" onClick={() => setModal("target")}><Plus size={15}/>Добавить</button></div></div>
              {targets.length === 0 && <div className="empty">Каналов пока нет</div>}
              {targets.map((item) => <button className={`entity-row entity-button ${selectedTarget?.target_id === item.target_id ? "selected" : ""}`} key={item.target_id} onClick={() => void openTarget(item)}><div className="entity-icon"><Radio size={18}/></div><div className="entity-main"><strong>{item.name}</strong><span>{item.platform} · {item.external_id}</span></div><span className={item.is_active ? "switch-label on" : "switch-label"}>{item.is_active ? "Активен" : "Выключен"}</span><ChevronRight size={16}/></button>)}
            </div>

            <div className="editor-panel channel-panel">
              {!selectedTarget ? <div className="empty large"><Radio size={34}/><h3>Выберите канал</h3><p>Здесь будут настройки канала и его источники.</p></div> : <>
                <div className="editor-head"><div><span className="badge">{selectedTarget.platform}</span><h2>{selectedTarget.name}</h2></div>{selectedTarget.url && <a href={selectedTarget.url} target="_blank" rel="noreferrer">Открыть <ExternalLink size={14}/></a>}</div>
                <div className="actions">
                  <button className="secondary" onClick={async () => { await api.updateTarget(selectedTarget.target_id, { is_active: !selectedTarget.is_active }); await loadAll(); }}>{selectedTarget.is_active ? "Отключить" : "Включить"}</button>
                  <button className="danger" onClick={async () => { if (!confirm("Удалить канал?")) return; await api.deleteTarget(selectedTarget.target_id); setSelectedTarget(null); setTargetSources([]); await loadAll(); }}><Trash2 size={15}/>Удалить</button>
                </div>
                <div className="section-divider" />
                <div className="table-head embedded"><h2>Источники канала</h2><span>{targetSources.length}</span></div>
                {targetSources.length === 0 && <div className="empty">Пока ничего не подключено</div>}
                {targetSources.map((link) => {
                  const source = sources.find((item) => item.source_id === link.source_id);
                  return <div className="entity-row" key={link.target_source_id}><div className="entity-icon"><Database size={17}/></div><div className="entity-main"><strong>{source?.name ?? `Источник #${link.source_id}`}</strong><span>{source?.url}</span></div><button className="icon-button" title="Отключить источник" onClick={async () => { await api.detachSource(selectedTarget.target_id, link.target_source_id); setTargetSources(await api.targetSources(selectedTarget.target_id)); }}><X size={16}/></button></div>;
                })}
                <div className="attach-list">
                  <label>Подключить источник</label>
                  {sources.filter((item) => !attachedSourceIds.has(item.source_id)).map((source) => <button key={source.source_id} className="attach-source" onClick={() => void attachSource(source.source_id)}><Link2 size={15}/><span>{source.name}</span><small>{source.platform}</small></button>)}
        ...
        {section === "sources" && (
          <section className="table-card">
            <div className="table-head"><h2>Источники</h2><div className="table-actions"><span>{sources.length} всего</span><button className="primary compact" onClick={() => setModal("source")}><Plus size={15}/>Добавить</button></div></div>
            {sources.length === 0 && <div className="empty">Источников пока нет</div>}
            {sources.map((item) => <div className="entity-row" key={item.source_id}><div className="entity-icon"><Database size={18}/></div><div className="entity-main"><strong>{item.name}</strong><span>{item.platform} · {item.url}</span></div><button className={item.is_active ? "switch-label on clickable" : "switch-label clickable"} onClick={async () => { await api.updateSource(item.source_id, { is_active: !item.is_active }); await loadAll(); }}>{item.is_active ? "Активен" : "Выключен"}</button><button className="icon-button danger-icon" title="Удалить" onClick={async () => { if (!confirm("Удалить источник?")) return; await api.deleteSource(item.source_id); await loadAll(); }}><Trash2 size={16}/></button></div>)}
          </section>
        )}
      </main>

      {modal && <div className="modal-backdrop" onMouseDown={() => setModal(null)}><div className="modal-card" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><p className="eyebrow">НАСТРОЙКА</p><h2>{modal === "target" ? "Новый канал" : "Новый источник"}</h2></div><button className="icon-button" onClick={() => setModal(null)}><X size={18}/></button></div>{modal === "target" ? <form onSubmit={(event) => void createTarget(event)} className="form-grid"><label>Название<input name="name" required placeholder="Новости 51 региона" /></label><label>Платформа<select name="platform" defaultValue="max"><option value="max">MAX</option><option value="telegram">Telegram</option><option value="vk">VK</option></select></label><label>ID канала<input name="external_id" required placeholder="-77162942582085" /></label><label>Ссылка<input name="url" placeholder="https://max.ru/..." /></label><button className="primary" disabled={busy}>Создать канал</button></form> : <form onSubmit={(event) => void createSource(event)} className="form-grid"><label>Название<input name="name" required placeholder="Полуостров 51" /></label><label>Платформа<select name="platform" defaultValue="vk"><option value="vk">VK</option></select></label><label className="wide">Ссылка<input name="url" required placeholder="https://vk.com/peninsula51" /></label><button className="primary" disabled={busy}>Добавить источник</button></form>}</div></div>}
    </div>
  );
}
