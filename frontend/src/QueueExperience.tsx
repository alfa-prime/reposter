...
const tabConfig: Record<QueueTab, { label: string; statuses: string[] }> = {
  storage: {
    label: "Хранилище постов",
    statuses: ["pending", "rewriting", "awaiting_moderation"],
  },
  scheduled: {
...
export function QueueExperience() {
  ...
  async function load(silent = false) {
    ...
    try {
      ...
      setItems(data);
      if (selectedId && !data.some((item) => item.queue_item_id === selectedId)) {
        setSelectedId(null);
      }
    } catch (exc) {
  ...
  async function uploadPhotos(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const file of files) {
        updated = await api.uploadQueuePhoto(selected.queue_item_id, file);
      }
      applyUpdated(updated);
  ...
  async function removeUploadedPhoto(photo: QueuePhoto) {
    ...
    await action(
      () => api.deleteQueuePhoto(selected.queue_item_id, photo.media_id!),
      "Фото удалено",
    );
  }
  ...
  return (
    <section className="editorial-queue">
      <div className="editorial-tabs" role="tablist" aria-label="Разделы очереди">
        ...
        <button className="editorial-refresh" onClick={() => void load()} disabled={busy} title="Обновить">
          <RefreshCw size={17} className={busy ? "spin" : ""}/>
        </button>
      </div>

      {(error || notice) && (
        <div className={error ? "editorial-message error" : "editorial-message"}>
      ...
      <div className="editorial-card-grid">
          ...
          <button className="editorial-post-card" key={item.queue_item_id} onClick={() => setSelectedId(item.queue_item_id)}>
            ...
            <div className="editorial-card-body">
              <div className="editorial-card-meta">
                ...
                <time>{shortDate(item.source_published_at)}</time>
              </div>
        ...
        {!visibleItems.length && (
          <div className="editorial-empty">
            ...
            <span>Посты появятся здесь по мере прохождения редакционного процесса.</span>
          </div>
        )}
      </div>
        ...
        <div className="editorial-drawer-backdrop" onMouseDown={() => setSelectedId(null)}>
          <aside className="editorial-drawer" onMouseDown={(event) => event.stopPropagation()}>
            <header className="editorial-drawer-head">
              ...
              <div>
                <h2>{selected.status === "scheduled" ? "Публикация в очереди" : "Публикация на модерации"}</h2>
                <div className={`editorial-status status-${selected.status}`}>{statusLabels[selected.status] ?? selected.status}</div>
              </div>
            </header>

            {selected.status === "scheduled" && selected.scheduled_at && (
              <div className="scheduled-banner"><CalendarClock size={18}/> Запланировано на {shortDate(selected.scheduled_at)}</div>
            )}

            <div className="drawer-source-row">
              <div>
                <strong>{selected.target_name ?? `Канал #${selected.target_id}`}</strong>
            ...
            <div className="drawer-media">
              {selected.photos.length > 0 ? (
                <div className="drawer-photo-grid">
                  {selected.photos.map((photo) => (
                    <div className="drawer-photo" key={`${photo.kind}-${photo.media_id ?? photo.attachment_id}`}>
                      <img src={photo.source_url} alt="Фото публикации" onClick={() => setLightbox(photo)}/>
                      {photo.kind === "uploaded" && photo.media_id && (
                        <button title="Удалить загруженное фото" onClick={() => void removeUploadedPhoto(photo)}><Trash2 size={15}/></button>
                      )}
                      {photo.kind === "uploaded" && <span>Добавлено вручную</span>}
                    </div>
                  ))}
                </div>
              ...
              <label className="upload-media-button">
                <Upload size={17}/>
                Добавить фото
                <input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={(event) => void uploadPhotos(event)} disabled={busy}/>
              </label>
              <small>JPEG, PNG или WebP · до 10 МБ на файл</small>
            </div>

            <div className="drawer-text-section">
              <div className="drawer-section-title">
                <span>Текст поста</span>
                <span>{draft.length} знаков</span>
              </div>

              {editing ? (
                <textarea value={draft} onChange={(event) => setDraft(event.target.value)} rows={12} autoFocus/>
              ) : (
            ...
            {selected.status === "approved" && (
              <div className="schedule-panel">
                <label>Дата и время публикации<input type="datetime-local" value={scheduleAt} onChange={(event) => setScheduleAt(event.target.value)}/></label>
                <button className="primary" onClick={() => void schedule()} disabled={busy}><CalendarClock size={17}/>Поставить в очередь</button>
              </div>
            )}
            ...
            <footer className="drawer-footer">
              <div className="drawer-main-actions">
                ...
                {["pending", "rewriting", "rejected"].includes(selected.status) && (
                  <button className="primary" onClick={() => void submit()} disabled={busy}><Send size={17}/>На модерацию</button>
                )}
                {selected.status === "awaiting_moderation" && (
                  <>
                    <button className="primary" onClick={() => void action(() => api.approve(selected.queue_item_id), "Пост согласован")} disabled={busy}><CheckCircle2 size={17}/>Согласовать пост</button>
                    <button className="danger" onClick={() => void action(() => api.reject(selected.queue_item_id), "Пост отклонён")} disabled={busy}><XCircle size={17}/>Отклонить</button>
                  </>
                )}
                {["approved", "scheduled", "rejected"].includes(selected.status) && (
                  <button className="secondary" onClick={() => void action(() => api.reopen(selected.queue_item_id), "Пост возвращён в работу")} disabled={busy}><RotateCcw size={17}/>Вернуть в работу</button>
                )}
              </div>
      ...
      {lightbox && (
        <div className="queue-lightbox" onClick={() => setLightbox(null)}>
          <button onClick={() => setLightbox(null)}><X size={24}/></button>
          <img src={lightbox.source_url} alt="Фото публикации" onClick={(event) => event.stopPropagation()}/>
        </div>
      )}
    </section>
  );
