...
export function TargetsPage({ targets, sources, selectedTarget, targetSources, busy, onOpenTarget, onChanged, onTargetSourcesChanged, onDelete, onError }: TargetsPageProps) {
  ...
  return (
    <>
      <section className="split-admin">
        <div className="table-card">
          <div className="table-head">
            <h2>Каналы</h2>
            <div className="table-actions"><span>{targets.length} всего</span><button className="primary compact" type="button" onClick={() => { onError(""); setCreateOpen(true); }}><Plus size={15} />Добавить</button></div>
          </div>
          {targets.length === 0 && <div className="empty">Каналов пока нет</div>}
          {targets.map((item) => (
            <button className={`entity-row entity-button ${selectedTarget?.target_id === item.target_id ? "selected" : ""}`} key={item.target_id} type="button" onClick={() => void onOpenTarget(item)}>
              <div className={`entity-icon ${item.icon_url ? "channel-avatar" : ""}`}>{item.icon_url ? <img src={item.icon_url} alt="" /> : <Radio size={18} />}</div>
              <div className="entity-main"><strong>{item.name}</strong></div>
              <span className={item.is_active ? "switch-label on" : "switch-label"}>{item.is_active ? "Активен" : "Выключен"}</span>
        ...
        <div className="editor-panel channel-panel">
          {!selectedTarget ? (
            <div className="empty large"><Radio size={34} /><h3>Выберите канал</h3><p>Здесь будут настройки канала и его источники.</p></div>
          ) : (
            <>
              <div className="editor-head">
                <div><span className="badge">{selectedTarget.platform}</span><h2>{selectedTarget.name}</h2></div>
                {selectedTarget.url && <a href={selectedTarget.url} target="_blank" rel="noreferrer">Открыть <ExternalLink size={14} /></a>}
              </div>
              <div className="actions">
                <button className="secondary" type="button" disabled={busy} onClick={() => void toggleTarget(selectedTarget)}>{selectedTarget.is_active ? "Отключить" : "Включить"}</button>
                <button className="danger" type="button" onClick={() => onDelete(selectedTarget)}><Trash2 size={15} />Удалить</button>
              </div>
              <ChannelRewriteSection target={selectedTarget} onChanged={onChanged} onError={onError} />
              <ChannelSignatureSection target={selectedTarget} onChanged={onChanged} onError={onError} />
      ...
      {createOpen && <CreateTargetModal busy={busy} onClose={() => setCreateOpen(false)} onError={onError} onCreated={async (target) => { await onChanged(); await onOpenTarget(target); }} />}
    </>
  );
}
