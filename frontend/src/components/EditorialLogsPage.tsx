...
export function EditorialLogsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [page, userData, targetData] = await Promise.all([
        api.editorialAuditEvents({ offset, limit, actorUserId: actor || undefined, targetId: target || undefined, action: action || undefined, dateFrom: dateFrom ? new Date(`${dateFrom}T00:00:00`).toISOString() : undefined, dateTo: dateTo ? new Date(`${dateTo}T23:59:59`).toISOString() : undefined }),
  ...
  }, [action, actor, dateFrom, dateTo, limit, offset, target, targets, users]);
  useEffect(() => { void load(); }, [load]);
  useEffect(() => { setOffset(0); }, [action, actor, dateFrom, dateTo, limit, target]);
