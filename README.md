...
Цель хранит платформу и внешний идентификатор канала. Для MAX это `chat_id`,
который приложение получает через webhook; токен бота остаётся в `.env` и в
...
Публикация в MAX выполняется только через редакционную очередь: пост нужно
подготовить, отправить на модерацию, одобрить и опубликовать сразу либо по
расписанию.
