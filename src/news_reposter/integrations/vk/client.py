...
class VKClient:
    ...
    def __init__(self, *, access_token: str, api_version: str = "5.199", api_url: str = "https://api.vk.com/method", http_client: httpx.AsyncClient | None = None) -> None:
    ...
    async def _request_group_info(self, client: httpx.AsyncClient, params: dict[str, str]) -> dict[str, object]:
        try:
            response = await client.get(f"{self._api_url}/groups.getById", params=params)
    ...
    async def get_posts_after(self, group: str, after_post_id: int) -> list[VKPost]:
        ...
        params["count"] = 100
        if self._http_client is not None:
            return await self._request_posts_after(self._http_client, params, after_post_id)
        async with httpx.AsyncClient(timeout=15.0) as client:
            return await self._request_posts_after(client, params, after_post_id)

    async def _request_latest(self, client: httpx.AsyncClient, params: dict[str, str | int]) -> VKPost | None:
    ...
    async def _request_posts_after(self, client: httpx.AsyncClient, params: dict[str, str | int], after_post_id: int) -> list[VKPost]:
        offset = 0
    ...
    async def _request_post(self, client: httpx.AsyncClient, params: dict[str, str | int]) -> VKPost | None:
        envelope = await self._request_wall(client, params)
        if envelope.response is None or not envelope.response.items:
            return None
        return envelope.response.items[0]

    async def _request_wall(self, client: httpx.AsyncClient, params: dict[str, str | int]) -> VKWallEnvelope:
        try:
        ...
        if envelope.error:
            raise VKAPIError(message=envelope.error.get("error_msg", "VK вернул ошибку"), code=envelope.error.get("error_code"))
        return envelope

    def _base_wall_params(self, group: str) -> dict[str, str | int]:
        params: dict[str, str | int] = {"access_token": self._access_token, "v": self._api_version, "filter": "owner"}
    ...
    def _group_identifier(group: str) -> str:
        ...
        if "://" in value:
            ...
            allowed_hosts = {"vk.com", "www.vk.com", "m.vk.com", "vk.ru", "www.vk.ru", "m.vk.ru"}
